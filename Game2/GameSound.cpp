...
GameSound::GameSound() {
    ...
    if (!_musicBackground.openFromFile("sounds/bgMusic.ogg")){
    ...
    // Chargement des sons
    if (!_swim.loadFromFile("sounds/swim.ogg")){
        std::cerr << "Score sound can't be loaded" << std::endl;
        std::exit(EXIT_FAILURE);
    }
    if (!_score.loadFromFile("sounds/score.ogg")){
        std::cerr << "Score sound can't be loaded" << std::endl;
        std::exit(EXIT_FAILURE);
    }
    if (!_lose.loadFromFile("sounds/lose.ogg")){
        std::cerr << "Score sound can't be loaded" << std::endl;
        std::exit(EXIT_FAILURE);
    }
    isSwimming = false;
